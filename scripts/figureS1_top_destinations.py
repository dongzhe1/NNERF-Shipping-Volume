...
def load_and_prepare_data(csv_file_path: str, top_n: int = 20):
    ...
    # Step 3: rank by total RF across all years & SSPs
    top_countries = (
        df_agg.groupby("DCountry")["RF"]
        .sum()
        .nlargest(top_n)
        .index.tolist()
    )
    top_countries = sorted(top_countries)  # alphabetical panel order
    print(f"Top {top_n} destination countries: {top_countries}")
    ...
    return plot_df, top_countries


def make_figure(plot_df, top_countries,
    ...
    # Fixed y-axis: 0 to 1,000,000 matching the original figure
    y_max   = 1_000_000
    ...
    fig.suptitle(
        "Top 20 Destination Countries by Total Traffic",
        fontsize=22, x=0.44, y=0.96
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(input_file):
        ...
        plot_df, top_countries = load_and_prepare_data(input_file, top_n=20)
        plot_df.to_csv("figureS1_plot_data.csv", index=False)
